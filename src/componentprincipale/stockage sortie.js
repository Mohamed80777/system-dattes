...
export default function StockageEntre() {
  ...
  // Initialisation de l'état avec les champs nécessaires
  const [formData, setFormData] = useState({
    ...
    gc: '',
    poids: '',
    chambre: 'chambre 1', // Valeur par défaut
    operation: 'sortie'   // Valeur fixe pour ce formulaire
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:8888/projet de stage/api/stockagesortie.php', // Assurez-vous que l'URL est correcte
        formData,
        {
          headers: {
      ...
      if (response.data.status === 'success') {
        alert("Stockage de sortie enregistré avec succès !");
        navigate('/stockage'); // Redirection après soumission réussie
      } else {
      ...
      <div className="container py-5">
        <div className="row justify-content-center">
          <div className="col-md-8 bg-white p-4 rounded">
            <h2 className="text-center mb-4">Stockage de Sortie</h2>
            {error && (
