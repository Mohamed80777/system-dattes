...
export default function TableChambre1() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`http://localhost:8888/projet de stage/api/chambre1.php?search=${search}`);
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Voulez-vous vraiment supprimer ce lot ?")) {
            try {
                const response = await axios.delete(`http://localhost:8888/projet de stage/api/chambre1.php?id=${id}`);
                if (response.data.status === 'success') {
